...
function insertAllPages(pages) {
    const insertPages = db.prepare("INSERT OR REPLACE INTO personal_pages (link, title, content) values (?, ?, ?)");
...
function insertPage(page) {
    //console.log(page);
    const insertPage = db.prepare("INSERT OR REPLACE INTO personal_pages (link, title, content) values (?, ?, ?)");
    return insertPage.run(String(page.link), String(page.title), String(page.content));
}

function insertAllLinks(pages) {
    const insertLinks = db.prepare("INSERT OR REPLACE INTO personal_links (origin, destination) values (?, ?)"); 
    const insertMany = db.transaction( (pages) =>{
        for(page in pages) {
...
function insertLink(origin, destination) {
    const insertLink = db.prepare("INSERT OR REPLACE INTO personal_links (origin, destination) values (?, ?)"); 
    console.log("aaaaaa "+ origin + " " + destination);
    return insertLink.run(origin, destination);
}

function insertWordCounts(link, wordCount) {
    const insertWordCount = db.prepare("INSERT OR REPLACE INTO personal_wordcount (link, word, frequency) values (?, ?, ?)"); 
...
function selectAllLinks() {
    const incomingLinks = db.prepare('SELECT origin, destination FROM personal_links').all();
    return incomingLinks
}

function selectPopular() {
    const query = db.prepare("SELECT COUNT(origin) as incoming, destination as link FROM personal_links GROUP BY destination ORDER BY incoming DESC LIMIT 10;");
    return query.all();
}

function selectLink(id) {
    const pageInfo = db.prepare('SELECT * FROM personal_pages WHERE link = ?').get(id); 
    const incomingLinks = db.prepare('SELECT Origin FROM personal_links WHERE Destination = ?').all(id);
    return {
        url: pageInfo.link,
...
function selectAllPages() {
    const query = db.prepare('SELECT * FROM personal_pages');
    return query.all();
}
...
function updatePageRanks(pages, pagerank) {
    const query = db.prepare('UPDATE personal_pages SET pagerank = ? WHERE link = ?');
